...
if "region_validee" not in st.session_state:
    st.session_state.region_validee = False

if "region_choisie" not in st.session_state:
    st.session_state.region_choisie = None
...
with title1 :
  ...
  with st.expander("Notes de version", expanded = False):
    ...
    st.write("08/05/2026 : Données rafraichies, modifications visuelles, liens cliquables sur la carte, ajout d’un code couleur pour les catégories (à vérifier / affiner)...")
    st.write("09/05/2026 : Affichage du créateur de la donnée, Affichage des résultats par date de mise à jour décroissante, Ajout des intercommunalités depuis https://www.data.gouv.fr/datasets/base-nationale-sur-les-intercommunalites. Choix de région en amont pour pouvoir en proposer plus (à venir) et améliorer les performances. Notes de versions masquées par défaut...")
...
liste_reg = ["Hauts_de_France", "Île_de_France", "Normandie"]

with st.form("choix_reg") :
    regs = sorted(liste_reg)
    reg = st.selectbox("Région :", regs)

    choix_region = st.form_submit_button("Choisir cette région")

    if choix_region:
        st.session_state.region_validee = True
        st.session_state.region_choisie = reg

if st.session_state.region_validee:
    #Chargement du DataFrame étudié :
    df = pd.read_csv(f"{st.session_state.region_choisie}.csv",
        dtype={
            "nom_region": "category",
    ...
    df = df.copy().sort_values(by = 'Date_de_mise_a_jour', ascending = False)
    ...
    with st.form("filtres"):
        ...
        col_types, col_dep, col_intercom, col_ville = st.columns(4)
                        ...
                        with st.container(border=True):
                            ...
                            with col3:             
                                for i in row['Categories_de_POI']:
                                    ...
                                    if i not in ['Lieu', "Point d'intérêt", "Produit"]:
                                        st.markdown(
